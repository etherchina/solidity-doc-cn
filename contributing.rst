...
另一个限制是,列表只会被解析一层，而不是递归解析。

下面是一个粗略的说明：

任何出现 ``<name>`` 的地方都会被所提供的变量 ``name`` 的字符串值所替换，既不会进行任何转义也不会进行迭代替换。可以通过 ``<#name>...</name>`` 来限定一个域。该域被它的内容的多次串联所替换，有多少组变量集提供到了模板系统中，就会进行多少次串联，每次串联都会用他们各自的值替换掉任何的 ``<inner>`` 项。（译者注：对于域<#name>...</name>的释义，译者参考自：https://github.com/janl/mustache.js#sections）。顶层变量也可以在这样的域的内部使用。
