...
.. note::
    |modifier| **可以被忽略**。
    以下特性仅实装在 0.4.0 版本之前的 Solidity 中：
...
::
    ...
    contract StateMachine {
        ...
        // This is the current stage.这是当前阶段。
        ...
        // Perform timed transitions. Be sure to mention执行基于时间的阶段转换。
        // this modifier first, otherwise the guards请确保首先声明这个修饰器，
        // will not take the new stage into account.否则新阶段不会被带入账户。
        modifier timedTransitions() {
            ...
            if (stage == Stages.RevealBids &&
                ...
                nextStage();
            // The other stages transition by transaction由交易触发的其他阶段转换
            _;
        }

        // Order of the modifiers matters here!这里的修饰器顺序非常重要！
        function bid()
            public
            payable
            timedTransitions
        ...
        {
            // We will not implement that here我们不会在这里实现实际功能（因为这仅是个代码示例，译者注）
        }
        ...
        // This modifier goes to the next stage这个修饰器在函数执行结束之后
        // after the function is done.使合约进入下一个阶段。
