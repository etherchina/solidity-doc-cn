...
一个合约A与另一个合约B的任何交互以及任何以太币交易的控制权都交给合约B。
...
为了避免重入，你可以使用下面撰写的 Checks-Effects-Interactions 模式：
...
- 目前无论是合约还是“外部账户”都不能阻止有人给它们发送以太币。
  合约可以对一个正常的转账做出反应并拒绝它，但还有些方法可以不通过创建消息来发送以太币。
...
大多数函数会首先做一些检查工作（例如函数从哪里被调用，参数是否在取值范围之内，它们是否发送了足够的以太币，用户是否具有通证等等）。
这些检查工作应该首先被完成。
