...
我们不会在这里解决所有的问题，但至少我们会展示如何进行委托投票，同时，计票又是 **自动和完全透明** 。

我们的想法是为每个选票创建一份合约，为每个选项提供简称。
然后作为合约的创造者——即主席，将给予每个独立的地址以投票权。

地址后面的人可以选择要么自己投票，要么委托给他们信任的人来投票。
...
::
    ...
    contract Ballot {
        ...
        function vote(uint proposal) public {
            ...
            // 如果 `proposal` 超过了数组的范围，则会自动抛出异常，并恢复所以的改动
...
当前，许多事务都需要为所有的参与者授权投票，你有没有更好的主意？
...
::
    ...
    contract SimpleAuction {
        ...
        /// 对拍卖进行出价，具体的出价值将包含在事务。
        /// 如果没有在拍卖中胜出，则返还出价。
        function bid() public payable {
            // 参数不是必要的。因为所有的信息已经包含在了事务中。
...
之前的公开拍卖在接下来的将被扩展为一个秘密竞拍。
秘密竞拍的好处是在投标结束前不会有时间压力。
在一个透明的计算平台上进行秘密竞拍听起来像是一个矛盾，但密码学可以实现它。

在 **投标期间** ，投标人实际上并没有发送她的出价，而只是发送出价一个哈希版本。
...
并且，这是特意的(除了高的出价，它甚至提供了一个显式的标志来放置无效的出价):
投标人可以通过设置几个高或低的无效出价来迷惑竞争对手。
...
::
    ...
    contract BlindAuction {
        ...
        /// 设置一个秘密竞拍。
        /// 已发送的以太币只有在出价在披露阶段被正确披露才会退还。
        /// 如果与出价一起发送的以太币至少为 “value” 且 “fake” 不为真，则出价有效。
        /// 将 “fake” 设置为真，并发送不确切但满足订金的的金额是隐藏实际出价方法。
        /// 同一个地址可以放置多个出价。
        ...
        // 这是一个 "内部" 函数， 意味着它只能在本合约内调用（或继承合约）
        ...
        function withdraw() public {
            ...
            if (amount > 0) {
                // 这里很重要，首先要设零值。
                // 因为，作为接收调用的一部分，
                // 接收者可以在 `transfer` 返回之前，可重新调用该函数。（可查看上面关于‘条件 -> 影响 -> 交互’的标注）
                pendingReturns[msg.sender] = 0;
...
::
    ...
    contract Purchase {
        ...
        {
            emit ItemReceived();
            // 首先需要修改状态，这很重要，因为包含 `send` 合约调用可以被再次调用
            state = State.Inactive;

            // 注意: 这实际上允许买方和卖方阻止退款 - 应该使用退款模式。
